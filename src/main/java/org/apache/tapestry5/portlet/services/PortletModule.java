...
public final class PortletModule {
	...
	public PortletApplicationInitializer build(
		...
		return builder.build(logger, PortletApplicationInitializer.class,
				PortletApplicationInitializerFilter.class, configuration,
				terminator);
	}
	...
	public PortletResponseRenderer buildPortletResponseRenderer(
		...
		return new PortletPageResponseRendererImpl(markupWriterFactory,
				markupRenderer, pageContentTypeAnalyzer, response,
				portletGlobals);
	}
	...
	public void contributeMarkupRenderer(
		...
		configuration.override("JavaScriptSupport", javaScriptSupport,
	...
	public void contributePageRenderRequestHandler(
			...
			PortletRequestGlobals globals, Logger log) {
		configuration.add("Portlet", new PortletRenderRequestFilterImpl(
				globals, portletHandler, log));
	}
	...
	public PortletPageRenderRequestHandler buildPortletPageRenderRequestHandler(
			...
			@Autobuild PortletPageRenderRequestHandlerImpl terminator,
			PipelineBuilder pipelineBuilder, Logger log) {
		return pipelineBuilder.build(log,
				PortletPageRenderRequestHandler.class,
	 ...
	 * {@linkplain org.apache.tapestry5.ioc.services.PipelineBuilder pipeline}
	 * around {@link PortletComponentEventRequestHandler}. Filters on the
	 * request handler are supported here as well.
	 */
	@Marker({ Portlet.class })
	public ComponentEventRequestHandler buildPortletComponentEventRequestHandler(
			List<ComponentEventRequestFilter> configuration,
			@Autobuild PortletComponentEventRequestHandler terminator,
			PipelineBuilder pipelineBuilder, Logger logger) {
	...
	public void contributePorletComponentEventResultProcessor(
		...
		configuration.add(URL.class, new ComponentEventResultProcessor<URL>() {
			public void processResultValue(URL value) throws IOException {
				(portletGlobals.getActionResponse()).sendRedirect(value
						.toExternalForm());
			}
		});
	...
	private ComponentEventResultProcessor constructComponentEventResultProcessor(
			Map<Class, ComponentEventResultProcessor> configuration,
			StrategyBuilder strategyBuilder) {
		Set<Class> handledTypes = CollectionFactory.newSet(configuration
				.keySet());

		// A slight hack!
		configuration.put(Object.class,
				new ObjectComponentEventResultProcessor(handledTypes));

		StrategyRegistry<ComponentEventResultProcessor> registry = StrategyRegistry
				.newInstance(ComponentEventResultProcessor.class, configuration);
	...
	/**
	 * Intercept FormSupport push calls and adapt it to prefix generated ids
	 * with portlet window id. Doing we assure that there will be no collision
	 * between control name on client side.
	 * 
	 * @param receiver
		...
		MethodAdvice advice = new MethodAdvice() {
			...
			public void advise(MethodInvocation invocation) {
				...
				if (FormSupportImpl.class.equals(instance.getClass())) {
					FormSupport adapter = new PortletFormSupportAdapter(
							(FormSupport) instance, globals.getPortletRequest()
									.getWindowID()
	 ...
	 * <dd>Values are stored in the the field's value is stored in the
	 * APPLICATION_SCOPE in PortletSession
	 * 
	 * </dl>
	 */
